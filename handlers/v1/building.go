...
var buildingModel *photo.Building

//BuildingList return building list
func BuildingList(c *gin.Context) {
	baseParamsStruct := model.BaseParams{}
	baseParamsMaps := map[string]interface{}{}
	listParamsStruct := photo.BuildingListParams{}
	...
	data, pagin, notFound := buildingModel.List(baseParamsMaps, listParamsMaps)
	if notFound {
...
//BuildingDetail return building detail
func BuildingDetail(c *gin.Context) {
	baseParamsStruct := model.BaseParams{}
	...
	data, err := buildingModel.Detail(detailParamsMaps)
